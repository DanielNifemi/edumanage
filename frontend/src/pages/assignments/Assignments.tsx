...
const Assignments = () => {
  ...
  const [currentUserRole] = useState<'teacher' | 'student'>(user?.role as 'teacher' | 'student' || 'student'); // Use user.role
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      console.log('Assignments Data from API:', assignmentsData); // Added for debugging
      setAssignments(Array.isArray(assignmentsData) ? assignmentsData : []); // Ensure assignments is always an array
      setCourses(Array.isArray(coursesData) ? coursesData : []);
      setStudents(Array.isArray(studentsData) ? studentsData : []);

      // Fetch submissions for each assignment
      const allSubmissions: AssignmentSubmissionData[] = [];
      if (Array.isArray(assignmentsData)) { // Add check here as well
  ...
  const handleCreateAssignment = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      const courseId = assignmentForm.course_id;
  ...
  const handleSubmitAssignment = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (submissionForm.submitted_text) {
        formData.append('submitted_text', submissionForm.submitted_text);
      }
      if (submissionForm.submitted_url) {
        formData.append('submitted_url', submissionForm.submitted_url);
      }
      if (submissionForm.submitted_file) {
        formData.append('submitted_file', submissionForm.submitted_file);
      }

      await assignmentsAPI.submitAssignment(selectedAssignment.id || '', formData);
  ...
  const handleGradeSubmission = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!submissionToGrade || !submissionToGrade.id) { // Added null check for submissionToGrade.id
      setError('No submission selected or submission ID is missing.');
      return;
    }

    try {
      await assignmentsAPI.gradeSubmission(submissionToGrade.id, { // submissionToGrade.id is now guaranteed to be a string
  ...
  const getAssignmentStatus = (assignment: AssignmentData) => {
    const now = new Date();
    // Add a check for assignment.due_date before creating a Date object
    const dueDate = assignment.due_date ? new Date(assignment.due_date) : null;
    
    if (currentUserRole === 'student') {
      const studentSubmission = submissions.find(s => 
        s.assignment_id === assignment.id && s.student_id === '1' // This would be current student ID
      );
  ...
  const getDaysUntilDue = (dueDateString: string | undefined) => {
    if (!dueDateString) return 'N/A'; // Handle undefined dueDateString
  ...
  const getSubmissionStats = (): StudentStats => {
    const totalAssignments = assignments.length;
    const submittedCount = submissions.filter(s => s.student_id === '1').length; // Current student
    const gradedCount = submissions.filter(s => s.student_id === '1' && s.grade !== null).length;
                  ...
                  <div className="grid gap-4 py-4">
                    ...
                    <div className="grid grid-cols-4 items-center gap-4">
                      ...
                      <Select
                        ...
                        onValueChange={(value) => setAssignmentForm(prev => ({ ...prev, course_id: value }))}
                      >
                    ...
                    <div className="grid grid-cols-4 items-center gap-4">
                      ...
                      <Select
                        ...
                        onValueChange={(value: 'file' | 'text' | 'url' | 'multiple') => 
                ...
                <CardContent className="p-6">
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-2xl font-bold text-yellow-600">{(stats as StudentStats).pendingCount}</p>
                    </div>
                ...
                <CardContent className="p-6">
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-2xl font-bold text-blue-600">{(stats as StudentStats).submittedCount}</p>
                    </div>
                ...
                <CardContent className="p-6">
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-2xl font-bold text-green-600">{(stats as StudentStats).gradedCount}</p>
                    </div>
                ...
                <CardContent className="p-6">
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-2xl font-bold text-purple-600">{(stats as TeacherStats).totalSubmissions}</p>
                    </div>
              ...
              <Card>
                <CardContent className="p-6">
                  <div className="flex items-center justify-between">
                    <div>
                      <p className="text-sm font-medium text-gray-600">Graded</p>
                      <p className="text-2xl font-bold text-green-600">{(stats as TeacherStats).gradedSubmissions}</p>
                    </div>
                    <Award className="h-8 w-8 text-green-600" />
                  </div>
                </CardContent>
                ...
                <CardContent className="p-6">
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-2xl font-bold text-orange-600">{(stats as TeacherStats).pendingGrading}</p>
                    </div>
                    ...
                    <CardContent className="p-6">
                      <div className="flex items-start justify-between">
                        <div className="flex-1">
                          ...
                          <p className="text-blue-600 font-medium mb-1">{course?.title}</p>
                          <p className="text-gray-600 text-sm mb-3">{assignment.description}</p>
                          
                          <div className="flex items-center gap-4 text-sm text-gray-500 mb-3">
                            <div className="flex items-center gap-1">
                              <Calendar className="h-4 w-4" />
                              Due: {assignment.due_date ? new Date(assignment.due_date).toLocaleDateString() : 'N/A'} {/* Handle undefined due_date */}
                            </div>
                            <div className="flex items-center gap-1">
                              <Clock className="h-4 w-4" />
                              <p className={`text-sm font-medium ${getDaysUntilDue(assignment.due_date).includes('Overdue') ? 'text-red-500' : 'text-green-500'}`}>
                          ...
                          {currentUserRole === 'student' ? (
                            ...
                            <>
                              ...
                              <Button 
                                size="sm" 
                                variant="outline" 
                                onClick={() => assignment.id && handleDeleteAssignment(assignment.id)}> {/* Handle undefined assignment.id */}
                        ...
                        <div className="flex items-start justify-between">
                          <div className="flex-1">
                            ...
                            <p className="text-gray-600 text-sm mb-3">
                              Submitted: {submission.submitted_at ? new Date(submission.submitted_at).toLocaleDateString() : 'N/A'} {/* Handle undefined submitted_at */}
                            </p>
              ...
              <div className="grid gap-4 py-4">
                <div className="grid grid-cols-4 items-center gap-4">
                  ...
                  <Input
                    ...
                    step="0.1"
                    min="0"
                    max={assignments.find(a => a.id === submissionToGrade?.assignment_id)?.total_points || 100}
                    value={gradeForm.grade}
                    onChange={(e) => setGradeForm(prev => ({ ...prev, grade: e.target.value }))}
                    className="col-span-3"
                    required
                    placeholder={`Out of ${assignments.find(a => a.id === submissionToGrade?.assignment_id)?.total_points || 100}`}
                  />
                </div>
                <div className="grid grid-cols-4 items-start gap-4">
